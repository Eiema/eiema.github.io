	...
	var 
		...
		methods = {
			init 	: function( options ) {
				// To bear:
				// I used the number to present the block index
				// and the token is "-"
				// so the index.html will also modify to tj_container-number
				var number = 1; 
				if( this.length ) {					
					var $container = $(this);
					var identity = $container.attr('id');
					var tokenIndex = identity.indexOf("-");
					number = identity.substring(tokenIndex + 1); 						
					// To 小秘書：
					//
					// 我把tj_container id的尾巴都加上數字讓js判定js判定為不同物件
					// 尾巴是r表示是本來的（之後的區塊我都在id尾巴加數字做區別，以後可以用數字一直新增區塊）
					// 就照原設定
					//
					// navL	: '#tj_prev',
					// navR	: '#tj_next',
					//
					// 若尾巴不是r是數字，就在這裡判別尾巴數字，例如，$('#tj_container2')呼叫的，可取最後一個字串‘2’
					// if(this.尾巴是2) 
					// settings 改成
					//
					//	 navL	: '#tj_prev'+'2',
					...
					return this.each(function() {
					   ...
					   //
					   // 這$wrapper裡要改成指定id
					   // 除非 $el.find('ul.tj_gallery') 會找到第二的區塊的 ul 
